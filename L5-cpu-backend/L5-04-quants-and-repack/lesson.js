...
const SCENES = [
/* ------------------------------------------------------ 1 同一份权重，两种排法：<span class="hl-a">类型号固定</span>，<span class="hl-b">布局可换</span> */
{
  kicker: "L5-04 · 全局",
  ...
  caption: "本课覆盖 6 个文件：quants.{c,h}（标量基线与 ABI）、repack.{c,h}（交错布局与 gemv/gemm）、traits.{c,h}（谁来认领这个 op）。模型加载期是否启用 repack 缓冲由 `--no-repack` 控制。",
  src: "ggml/src/ggml-cpu/quants.h",
  mark: [6, 13, 14],
  lineNo: 8,
  code: `// GGML CPU internal header
...
void quantize_row_q8_1(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t k);`,
  ...
  build(root, tl) {
    ...
    defs.forEach((_, i) => tl.at(700 + i * 3400, () => {
      ...
      U.markLines(document, [6,13]);
    }));
    tl.at(12400, () => {
      els.forEach(e => e.style.opacity = '1');
      msg.innerHTML = texts[3];
      U.markLines(document, [6,13,14]);
    });
  }
},

/* ------------------------------------------------------ 2 基线：标量点积要<span class="hl-c">先拆 nibble</span> */
{
  kicker: "L5-04 · 基线",
  ...
  mark: [0, 11, 12, 22, 24, 26, 27],
  lineNo: 225,
  code: `void ggml_vec_dot_q4_0_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    const int qk = QK8_0;
    const int nb = n / qk;
...
//>> 低半是元素 j、高半是元素 j+16 —— 一个字节必须先拆成两个 int8
...
/* ------------------------------------------------------ 3 ★ <span class="hl-a">repack</span>：交错布局从类型号里搬进后端 */
{
  kicker: "L5-04 · 核心",
...
/* ------------------------------------------------------ 4 <span class="hl-d">make_block_q4_0x4</span>：4 行轮流搬，顺手异或掉偏置 */
{
  kicker: "L5-04 · 重排",
  ...
  code: `static block_q4_0x4 make_block_q4_0x4(block_q4_0 * in, int blck_size_interleave) {
    ...
    if (blck_size_interleave == 8) {
        ...
        for (int i = 0; i < end; ++i) {
            ...
            elems ^= xor_mask;
//>> xor 0x88：Q4_0 存的是无符号 4-bit 码（0..15），异或后变成 4-bit 补码 —— 内核里 << 4 / & 0xF0 直接得到有符号值，省掉减 8
...
/* ------------------------------------------------------ 5 交错是<span class="hl-c">双边</span>的：激活也要按 4 行一组量化 */
{
  kicker: "L5-04 · 契约",
  ...
  lineNo: 135,
  code: `void ggml_quantize_mat_q8_0_4x4_generic(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(QK8_0 == 32);
    assert(k % QK8_0 == 0);
    const int nb = k / QK8_0;

    block_q8_0x4 * GGML_RESTRICT y = (block_q8_0x4 *) vy;
//>> 目标类型是 block_q8_0x4 而不是 block_q8_0 —— 4 行的量化结果直接写成交错块
...
/* ------------------------------------------------------ 6 ★ 交错块怎么读：<span class="hl-a">偏移 = k·N·G + j·G + i</span> */
{
  kicker: "L5-04 · 收益",
  title: "★ 交错块怎么读：<span class=\"hl-a\">偏移 = k·N·G + j·G + i</span>",
  sub: "k 走列、j 走行、i 走行内字节（本例 N = G = 4）—— 一条向量载入就同时拿到多行的同一批列。",
  caption: "这是 generic 实现（可读版，第 j 行权重配第 m 个 token，一次出 16 个部分和）；x86 / ARM 上的真身是 AVX2 / NEON-i8mm 版本，见 L5-05。",
  src: "ggml/src/ggml-cpu/repack.cpp",
  mark: [20, 25, 27, 31, 32, 33, 34, 35, 36, 37, 39, 40, 41, 43],
  lineNo: 1785,
  code: `void ggml_gemm_q4_0_4x4_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int qk = QK8_0;
    const int nb = n / qk;
...
//>> << 4 把低 nibble 挪到高位、保住它的符号位；& 0xF0 原地取高 nibble —— 两者都已是"值-8"的补码
...
/* ------------------------------------------------------ 7 <span class="hl-d">traits</span>：把 op 从默认路径接过来的钩子 */
{
  kicker: "L5-04 · traits",
  ...
  mark: [1, 2, 5, 6, 9, 12, 13],
  lineNo: 18,
  code: `namespace ggml::cpu {
...
/* ------------------------------------------------------ 8 变体选择：<span class="hl-b">ISA + 形状</span> 决定 4x4 / 4x8 / 8x8 / 16x1 */
{
  kicker: "L5-04 · 选择",
...
/* ------------------------------------------------------ 9 谁先认领这个 op —— 与"变体 -> SIMD 宽度"总表 */
{
  ...
  mark: [0, 1, 3, 4, 5, 6, 10],
  lineNo: 12,
  code: `bool ggml_cpu_extra_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * op) {
