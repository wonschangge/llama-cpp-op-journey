...
3. 说明 `ggml_cann_create_tensor` 抹平了 ggml 与 CANN 之间的哪两处约定差异，并说出它为什么要重算存储长度；
...
1. **CANN 后端：7 个文件、10058 行、两个公共入口** — 华为昇腾（Ascend）NPU 的 ggml 后端，行数为 wc -l。七个文件分工明确：一个交契约、一个做分派、一个翻译算子。
...
6. **ggml_tensor -&gt; aclTensor：两处约定差异** — ggml 的 nb 以字节计、ne[0] 在最前；CANN 的 stride 以元素计、最内层维在最后。转换函数把这两处抹平。
...
- [x] 保真门禁：20 处引用 —— 20 个引用块 / 52 个连续段逐字命中
- [x] 覆盖度门禁：本课声明 7 项，无空课、无幻影；全局覆盖 1290/1290
