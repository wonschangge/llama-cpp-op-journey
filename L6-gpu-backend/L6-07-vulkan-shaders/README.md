...
2. 解释为什么 `string_to_spv` 这一层是"变体工厂"——同一个 `.comp` 会被编译成很多个 SPIR-V；
...
7. **mul_mmq.comp：B 侧先量化成 q8_1，再整数点积** — 三个矩阵乘族分工不同：mul_mm 按 M/N 分块，mul_mat_vec 的表按列数索引，mul_mmq 走整数点积。
