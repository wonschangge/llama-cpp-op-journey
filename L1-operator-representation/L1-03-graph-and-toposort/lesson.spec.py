...
注意（为什么 visual 写得这么紧凑）：tools/check_flags.py 的 strip_verbatim() 把
lesson.js 的【内容】当成路径传给 dump_scenes.js，node 回显整份内容到 stderr，
而该管道在 65536 字节处截断（实测 stderr = 2 * 文件字节 + 51）。
截断点若落在多字节字符中间，Python 的 text=True 解码就会抛 UnicodeDecodeError。
所以本课的 lesson.js 必须留在 32742 字节以内 —— 讲不完的细节放 source.md。
"""
...
L.scene(
    ...
    caption='跨课：src[] 是图的边（L1-01）；op 决定这个张量算节点还是叶子（L1-02）；'
            '这里排好的顺序最终被 L4-04 的 graph_compute 消费。',
...
L.scene(
    ...
    notes={8: 'used[pos] 已经是 1 = 这张图收过它了 —— 直接 return，不再递归、不再入列',
           23: '首次见到：先把指针写进 keys[]，再把 used 位置 1，最后清 use_counts'},
...
L.scene(
    ...
    notes={0: 'src[] 的长度上限 GGML_MAX_SRC = 10（见 L1-01）',
           11: 'use_counts 也按 hash 槽位索引：一次遍历顺手记下"每个张量被用了几次"'},
...
L.scene(
    ...
    caption='对照 L1-01（struct ggml_tensor 逐字段）：那 12 个字段里没有任何"我在图上被访问过"的标记。'
            'used[] 的位运算实现见 source.md 第一节。',
    src=IMPL, parts=[(235, 242)], duration=24000,
    notes={5: '位图：第 i 位为 1 表示槽位 i 已被占用（位运算实现见 source.md 第一节）'},
    marks=[0, 1, 3, 4, 5, 7],
    visual='''
const w = U.el('div', { class: 'col', style: 'gap:9px;width:100%' });
...
L.scene(
    ...
    caption='需要视图的数据来源时，代码必须自己沿 view_src 链上溯 —— 例如子图融合判定；'
            '外部的 view_src 只允许是常量权重。',
...
L.footnote_add('`ggml_graph_plan()` / `ggml_graph_compute()` 这一版不在 `ggml.c` 里'
               ...
               '不引用其源码，具体展开见 L4-04。')
L.footnote_add('第 7 幕提到的 GGML_DEFAULT_GRAPH_SIZE（默认图容量）定义在 `ggml/include/ggml.h`，'
               '该文件由 L1-01 覆盖；本课不引用其行号，故不计入本课声明。')
L.footnote_add('工程备注：`tools/check_flags.py` 的 `strip_verbatim()` 会把 lesson.js 的**内容**'
               '当作路径传给 `dump_scenes.js`，node 因此把整份内容回显到 stderr，而该回显在 65536 '
               '字节处被截断（实测 `stderr 长度 = 2 x 文件字节 + 51`）。截断点落在多字节字符中间时，'
               'Python 的 `text=True` 解码会抛 `UnicodeDecodeError`，整个参数门禁失败。'
               '本课的 `visual` 因此写得紧凑，把 lesson.js 控制在 32742 字节以内。')
