...
9. **24 个文件里，有 6 个名实不符** — 分组依据是"图构建代码命中了 MoE 图原语"。命中了不等于"这个文件就是 MoE"。
...
家族的差异全部落在 `build_moe_ffn` 的 23 个参数上：有没有 `gate_exps`、`gating_op` 取哪个枚举、`norm_w` / `w_scale` 传什么、有没有 `gate_up_exps`、以及 `hparams.n_expert_groups` 是否大于 1。**没有谁另写一张图。**
...
- [x] 保真门禁：39 处引用 —— 39 个引用块 / 80 个连续段逐字命中
- [x] 覆盖度门禁：本课声明 25 项，无空课、无幻影；全局覆盖 113/1290
- [x] 参数门禁：真值集 381 长 / 76 短选项，扫描 54 文件 9 处引用，0 处非法
- [x] 语法检查：0 错误 / 0 警告
