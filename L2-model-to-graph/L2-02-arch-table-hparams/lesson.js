...
/* ------------------------------------------------------ 2 <span class="hl-a">153</span> 条名字，一张"身份证"表 */
{
  kicker: "L2-02 · 架构表",
  ...
  mark: [0, 1, 3],
  lineNo: 8,
  code: `static const std::map<llm_arch, const char *> LLM_ARCH_NAMES = {
    { LLM_ARCH_CLIP,             "clip"             }, // dummy, only used by llama-quantize
//>> 第 1 行的注释点明：clip 是 dummy，只给 llama-quantize 用
...
/* ------------------------------------------------------ 3 ★ <span class="hl-c">%s</span>：GGUF 的 key 是"算"出来的 */
{
  kicker: "L2-02 · 核心",
  ...
  mark: [2, 3],
  lineNo: 1000,
  code: `LLM_KV::LLM_KV(llm_arch arch, const char * suffix) : arch(arch), suffix(suffix) {}

std::string LLM_KV::operator()(llm_kv kv) const {
    std::string name = ::format(LLM_KV_NAMES.at(kv), LLM_ARCH_NAMES.at(arch));
//>> ::format 把 LLM_KV_NAMES 的模板套上 LLM_ARCH_NAMES 的架构名
...
/* ------------------------------------------------------ 5 名字 = <span class="hl-c">format</span>(模板, bid, xid) */
{
  kicker: "L2-02 · 张量命名",
  ...
  mark: [0, 6],
  lineNo: 1016,
  code: `std::string LLM_TN_IMPL::str() const {
//>> str() 是 LLM_TN_IMPL 的唯一出口：张量名在这里才真正生成
...
//>> bid 是层号，xid 是专家号；模板里有几个 %d，就用几个
    if (suffix != nullptr) {
        name += ".";
        name += suffix;
    }
...
/* ------------------------------------------------------ 6 <span class="hl-d">llama_hparams</span>：模型文件说什么，就是什么 */
{
  ...
  mark: [0, 2, 3, 10],
  lineNo: 64,
  code: `    uint32_t n_ctx_train; // context size the model was trained on
//>> 注意是 n_ctx_train：训练时的长度，不是这次推理用多长
    uint32_t n_embd;
    uint32_t n_layer_all;
//>> n_layer_all 含 nextn 预测层；n_layer() 返回的是"实际要跑的层数"
...
/* ------------------------------------------------------ 7 ★ 每个形状字段都有 <span class="hl-e">(il)</span> 版本：逐层可以不同 */
{
  kicker: "L2-02 · 核心",
  ...
  mark: [0, 8, 11],
  lineNo: 99,
  code: `uint32_t llama_hparams::n_gqa(uint32_t il) const {
    const uint32_t n_head    = this->n_head(il);
    const uint32_t n_head_kv = this->n_head_kv(il);

    if (n_head_kv == 0) {
//>> 有些层根本没有 KV（纯 SSM 层），这里返回 0 而不是除零
        return 0;
    }
...
//>> 同一个 getter，SWA 层和全注意力层返回不同的 RoPE 维度
    if (il < n_layer_all) {
...
/* ------------------------------------------------------ 8 <span class="hl-b">llama_cparams</span>：一次运行的可变配置 */
{
  ...
  mark: [0, 1, 4, 6, 11],
  lineNo: 10,
  code: `struct llama_cparams {
    uint32_t n_ctx;           // context size used during inference
//>> 对照 hparams.n_ctx_train：一个来自模型文件，一个来自调用者
    uint32_t n_ctx_seq;       // context for a single sequence
    uint32_t n_batch;
//>> n_batch 是一次送进图的 token 上限，n_ubatch 是其中 micro-batch 的大小
    uint32_t n_ubatch;
    uint32_t n_seq_max;
...
/* ------------------------------------------------------ 9 新增一个架构，要动哪几张表 */
{
  ...
  mark: [0, 1],
  lineNo: 1061,
  code: `bool llm_arch_is_recurrent(const llm_arch & arch) {
    switch (arch) {
        case LLM_ARCH_MAMBA:
