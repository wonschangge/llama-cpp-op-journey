...
4. 判断一个 `src/models/*.cpp` 是不是"借图"（一个 `using graph =` 别名），以及这对读代码意味着什么。
...
7. **借图：3 个文件自己没有 MoE 图** — phimoe.cpp 是这 24 个文件里最短的一个，全篇没有一行 build_moe_ffn —— 它的 MoE 行为由 phi3.cpp:153 决定。
...
- [x] 覆盖度门禁：本课声明 29 项，无空课、无幻影；全局覆盖 879/1290
