...
| **高亮用 `mark_src`** | `mark_src=[686, 690]` = 按**上游行号**高亮，lessonkit 自动换算。**不要用 `marks=`**（渲染下标），插入注解行后会错位，lessonkit 也会拒绝指向空行/注解行的 `marks`。 |
| **注解用 `notes_src`** | `notes_src={686: '这一行是……'}` = 注解插在**上游第 686 行之后**，自动换算且会校验行号在引用区间内。`notes=`（块内下标）仍可用但不推荐。 |
| **注解不占行号** | 注解行在行号槽里显示 `>>`，**不消耗上游行号** —— 否则其后所有行号会整体错位，而"在哪一行"常常正是这一课的重点。 |
