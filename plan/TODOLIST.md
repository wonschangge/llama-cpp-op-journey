...
- [x] **`L2-11`** 模型家族（二）：状态空间与线性注意力　`P0`　30 文件
...
- [x] **`L2-15`** 模型家族（六）：稠密 Transformer（下）与投机解码草稿模型　`P0`　36 文件
...
- [x] **`L3-01`** 后端接口 ggml-backend.h：后端的契约　`P0`　2 文件
...
- [x] **`L4-01`** 分配器 ggml-alloc：算子的内存从哪来　`P0`　2 文件
...
- [x] **`L4-03`** buffer 与 buffer type　`P1`　2 文件
...
- [x] **`L4-04`** 图执行入口：graph_compute 与异步　`P1`　3 文件
...
- [x] **`L5-01`** CPU 后端骨架：从 graph_compute 到算子分派　`P0`　3 文件
...
- [x] **`L5-05`** ★ 多架构 SIMD 与厂商加速　`P0`　49 文件
...
- [x] **`L6-03`** ★ CUDA FlashAttention　`P0`　15 文件
...
- [x] **`L6-04`** CUDA 其余算子与模板实例化　`P1`　280 文件
...
- [x] **`L6-07`** Vulkan 后端：计算着色器　`P1`　162 文件
...
- [x] **`L8-01`** ★ 端到端：一个 mul_mat 的完整旅程　`P1`　1 文件
